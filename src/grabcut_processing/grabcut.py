...
def prep_prob_mask_for_gc(mask: np.ndarray, scribble_mask: np.ndarray, threshold_lo=0.1, threshold_hi=0.9, threshold_normal=0.5) -> np.ndarray:
    """
    Prepare mask for grabCut algorithm postprocessing.
    ...
    mask_gc[np.logical_and(no_scribble, mask >= threshold_hi)] = cv2.GC_FGD
    mask_gc[np.logical_and(no_scribble, mask <= threshold_lo)] = cv2.GC_BGD
    ...
    mask_gc[np.logical_and(not_definite, mask > threshold_normal)] = cv2.GC_PR_FGD
    mask_gc[np.logical_and(not_definite, mask <= threshold_normal)] = cv2.GC_PR_BGD
